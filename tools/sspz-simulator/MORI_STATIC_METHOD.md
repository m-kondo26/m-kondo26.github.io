...
Version 2026-09-18.3. Implementation: `mori-static-core.js`.
...
以下の数値は、設定を指定せずに別ページを開く場合の参照条件である。本体から開く場合は、現在の列数N（1～320）、1列幅d（列ピッチと開口幅に使用）、R、D、体軸方向の実効焦点寸法、評価半径、view数、z刻みを引き継ぐ。各列に対応するN個の固定再構成位置について計算し、現在の評価半径と回転中心で比較する。寝台移動は0に固定し、本体の補間方式、幅Tの平均化、面内の有限開口と再配列、z-FFSは引き継がない。したがって、本体の補間方式の選択に依存しない独立した参照計算として表示する。

本書はFeldkamp法の逆投影を踏まえて説明しているが、本実装は列間補間・距離重み・全周合算を取り出した体軸応答モデルであり、Feldkamp画像再構成全体ではない。
...
1/Mは一回転の平均という共通係数であり、最終等面積正規化で相殺される。アニメーションの途中では、全取得角度で求めた最終面積を分母として使用する。途中の曲線をその都度面積1にしてしまうと「積み重なる様子」が消えるため、その処理はしない。
...
`node tests/mori-static-rows.mjs`：1・4・80・160・320列で、全列の物理的な間隔・開口、全支持域、正規化、独立したモーメント計算、端処理、同期／逐次API一致を確認する。既定16列および320列の計算結果は18.2の保存ハッシュと完全一致。18.3は角度幾何の再利用により計算量を削減し、列ごとに進捗を返す `moriStaticCalculateSteps` を追加した。view数やz刻みを自動で粗くする処理は行わない。
