...
function selectFdkState(index,immediate=false){
  ...
  for(const id of ['fdk-geometry','fdk-weights-cba','fdk-weights-rri','fdk-axial','fdk-coronal'])drawCanvasStatus(document.getElementById(id),'',fdkText('','Computing selected angle'));
...
function drawFdkCandidateDiagram(canvas,r,zoom,reference=false){
  ...
  const diagram={totalRows:rows,z0:r.zObject,overviewXLimit:extent,zoomXLimit:zoomLimit,
    ...
    yAxisLabel:r.reference?fdkText('','Rebinned angle offset  θ  (°)'):fdkText('','Source angle offset  β  (°)'),
    directLegendLabel:r.reference?fdkText('','Rebinned data ○'):fdkText('','Acquired data ○'),
  ...
  if(!zoom)diagram.directLegendLabel=r.reference?fdkText('','Rebinned row trajectories'):fdkText('','Detector-row trajectories');
  if(!r.reference&&zoom){
...
function initializeFdkUi(initial){
  ...
  <p class="section-summary">${fdkText('','Reconstruct local 3D images and model SSPz from ideal-point projections. The Hsieh path compares quadratic CBA weights with a linear reference. The linear reference equals conventional RRI in the four-sample interior; both use the same edge treatment.')}</p>
...
versionLabel.textContent = `Web build 2026-09-17.2 / axial model ${MODEL_VERSION} / FDK 2026-09-17.2 / CBA 2026-09-17.2`;
...
function drawDiagramFamilyLegend(ctx, diagram, left, y, width) {
  ...
  const items = [{ label: diagram.directLegendLabel ?? localizedText("Direct-ray candidate ○", "Direct ○"), dashed: false, color: INK }];
