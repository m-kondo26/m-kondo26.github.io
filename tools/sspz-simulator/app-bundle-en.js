...
function drawOverviewLegend(ctx, diagram, left, top, width, countText) {
  ...
  ctx.textBaseline = "middle";
  ctx.fillStyle = INK;
  ctx.fillText("Detector row", left, top + 21);
  ...
  ctx.fillStyle = INK; ctx.fillText("Acquired projection data", left + 52, top + 54);
  ctx.strokeStyle = RED;
  ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(left + 256, top + 42); ctx.lineTo(left + 256, top + 66); ctx.stroke();
  ctx.fillStyle = INK; ctx.fillText("Target plane (zᵢ-z₀=0)", left + 268, top + 54);
  if (countText) {
    ctx.fillStyle = MUTED;
    ctx.font = `18px ${FIGURE_FONT}`;
