...
function initializeFdkUi(initial){
  ...
  </div><p class="model-note">${fdkText('各断面に1回転分を使用します。Hsieh法は同じ前処理・同じ投影からCBAとRRIを計算します。取得ビュー数は偶数にしてください。必要な投影や対向データが検出器範囲から外れる条件では計算を停止します。','Each slice uses one turn. The Hsieh path computes CBA and RRI from identical projections and preprocessing. Use an even view count. Computation stops if required projections or conjugate samples fall outside the detector.')}</p>`;
  ...
  <p class="section-summary">${fdkText('球の投影データから局所3次元画像とSSPzを計算します。Hsieh法では、CBA（対向データを同時に補間）とRRI（各投影内で列間補間）を比較します。','Reconstruct local 3D images and SSPz from sphere projections. The Hsieh path compares CBA (joint conjugate interpolation) with RRI (row interpolation within each view).')}</p>
  ...
  panel.insertAdjacentHTML('beforeend',`<div id="cba-samples-wrap" class="chart-card" hidden><h3>${fdkText('補間に使うサンプルと重み：球の中心位置','Interpolation samples and weights at the sphere center')}</h3><canvas id="cba-samples" width="1200" height="700"></canvas><p>${fdkText('灰：RRI、赤：CBA。点の面積は正規化した重みです。横軸は球の中心からの距離。各対向ペアの4点を、再配列後の角度で示します。これは中心位置の局所的な重みであり、SSPz全体の寄与率ではありません。','Gray: RRI; red: CBA. Marker area represents normalized weight. The four samples in each conjugate pair are shown at the rebinned angle and relative to the sphere center. These are local weights at the central point, not total contributions to SSPz.')}</p></div><p><a href="CBA_METHOD.md">${fdkText('Hsieh法：計算方法と適用範囲','Hsieh path: equations and scope')}</a> · <a href="https://doi.org/10.1117/1.2746866" target="_blank" rel="noopener noreferrer">Hsieh et al. (2007)</a></p>`);
...
function renderFdkResult(r){
  const c=r.config;document.getElementById('fdk-summary').textContent=r.reference?fdkText('Hsieh法：CBAとRRIの比較が完了しました。投影・前処理・画像格子は共通です。画像と半値幅の矢印はCBAの最初の角度条件です。','Hsieh comparison complete: CBA and RRI share projections, preprocessing and image grids. Images and the FWHM arrow show CBA at the first start angle.'):fdkText('再構成完了。各断面の全360°の取得範囲を確認しました。','Reconstruction complete. Full-turn acquisition coverage was checked for every slice.');
  ...
  if(r.reference){comparison.innerHTML=`<table><caption>${fdkText('各SSPzから求めたFWHM','FWHM computed from individual SSPz profiles')}</caption><thead><tr><th>${fdkText('補間方法','Method')}</th><th>${fdkText('平均 (mm)','Mean (mm)')}</th><th>SD (mm)</th><th>${fdkText('範囲 (mm)','Range (mm)')}</th></tr></thead><tbody>${fdkGroups(r).map(([name,g])=>{const q=fdkWidthStats(g);return `<tr><td>${name}</td><td>${q.mean.toFixed(2)}</td><td>${q.sd===null?'—':q.sd.toFixed(3)}</td><td>${q.min.toFixed(2)}–${q.max.toFixed(2)}</td></tr>`;}).join('')}</tbody></table>`;cbaDrawSamples(document.getElementById('cba-samples'),r);}
...
function drawDetectorRowLegend(ctx, diagram, left, y, width) {
  ...
  const label = localizedText("色：検出器列", "Color: detector row");
...
function drawWeightLegend(ctx, left, top, width, diagram, countText) {
  ...
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  const weightLabel = localizedText("選択候補の合計重み w (FW=0)", "Local total weight w (FW=0)");
