...
function runSimulation() {
  ...
  worker.onmessage = event => {
    ...
    if (message.type === "progress") {
      ...
      progress.value = 1;
      status.textContent = `完了 ${elapsed.toFixed(1)}秒 / 最近接2候補の線形補間 / 設定厚=${fmt(lastResult.params.sliceThicknessMm, 3)} mm / 閲覧状態${selectedStateIndex}の最大Δz/T=${fmt(lastResult.selectedOff.bracketGapRatioMax, 3)}・${fmt(lastResult.selectedOn.bracketGapRatioMax, 3)}`;
      setBusy(false);
...
function drawWeightLegend(ctx, left, top, width, totalRows, countText) {
  ...
  ctx.fillStyle = INK; ctx.fillText("0～360°の実取得データ", left + 53, y0 + 76);
  if (countText) {
    ctx.fillStyle = MUTED;
    ctx.font = `18px ${FIGURE_FONT}`;
...
function drawDiagram(canvas, diagram, mode = "zoom", sharedXLimit = null, focusXLimit = null) {
  ...
  if (mode === "overview") {
    ...
    ctx.font = `18px ${FIGURE_FONT}`;
    ctx.textAlign = "center";
    ctx.fillText("2Bの拡大範囲", x(0), margin.top + 24);
  } else {
    ctx.fillStyle = "#557482";
    ctx.font = `18px ${FIGURE_FONT}`;
    ctx.textAlign = "center";
    ctx.fillText("最近接2候補がz₀を挟む範囲", x(0), margin.top + 24);
  }
  ctx.save();
  ctx.beginPath();
  ...
  if (mode === "overview") {
    const countText = `全${diagram.totalRows}列表示（凡例は代表${Math.min(6, diagram.totalRows)}列）／基準回転との差 ${diagram.turnOffsetMin}〜${diagram.turnOffsetMax}（${diagram.turnCount}回転）／候補線 ${diagram.candidateLineCount}本`;
    ...
    const countText = `最近接候補 ${diagram.weightedPoints.length}点（${diagram.renderedAngleSamples}/${diagram.samples}角度表示）／寄与回転 ${usedTurnText}${outside}`;
...
function drawGapMap(canvas, result, coneOn) {
  ...
  plot.ctx.fillText(`最近接候補間隔比 Δz/T — ${conditionLabel}`, plot.margin.left, 10);
  if (!publicationMode) {
    plot.ctx.fillStyle = MUTED;
    plot.ctx.font = `18px ${FIGURE_FONT}`;
    plot.ctx.fillText(`設定厚 T=${fmt(result.params.sliceThicknessMm, 1)} mm／固定支持幅なし／灰色のみ欠測`, plot.margin.left, 43);
  }
...
function renderSummary(result) {
  ...
  for (const [key, label, css] of [["off", "コーン幾何を反映しない", ""], ["on", "コーン幾何を反映する", "on"]]) {
    const summary = result.summaries[key];
    primaryCards.push(`
      <div class="summary-card ${css}">
        <span>${label} / 全角度・全状態の最大 Δz/T</span>
        <strong>${fmt(summary.bracketGapRatioMax.max, 3)}</strong>
        <small>状態別最大値 ${fmt(summary.bracketGapRatioMax.min, 3)}–${fmt(summary.bracketGapRatioMax.max, 3)}</small>
      </div>`);
    secondaryCards.push(`
      <div class="summary-card ${css}">
...
function renderInspectionDetails(result) {
  ...
  overviewScope.textContent = `自動表示：z₀より小さい側・大きい側の最近接候補を含む回転と前後1回転（基準回転との差 ${result.diagramOff.turnOffsetMin}〜${result.diagramOff.turnOffsetMax}、合計${result.diagramOff.turnCount}回転）`;
  calculationScope.textContent = `0～360°の実取得データ1系列・全${result.params.rows}列、候補線${result.diagramOff.candidateLineCount}本／各角度で最近接2候補の線形重みを正規化`;
...
function renderAll(result) {
  ...
  if (overlayScope && result.overlay) {
    overlayScope.textContent = `1回転寝台移動量を${result.overlay.stateCount}等分（1°相当）／各状態を0～360°の全${result.params.viewSamples}ビューで計算`;
  }
...
if (legacyUrlNote) {
  ...
  if (legacyInputMigrated) legacyUrlNote.textContent = "旧URL・保存条件を新方式へ移行しました。旧版の180°ハーフスキャン計算値は流用せず、0～360°の実取得データでSSPzを再計算するため、旧版と同じ結果にはなりません。";
}
runSimulation();
