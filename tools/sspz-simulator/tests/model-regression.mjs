...
assert.match(indexSource, /id="filterWidthMm"[^>]*type="hidden"/);
assert.match(indexSource, /id="sliceThicknessMm"[^>]*min="0.1"[^>]*max="20"/);
assert.match(indexSource, /id="filterSamples"/);
assert.match(indexSource, /入力したTを体軸方向の矩形平均化幅として使います/);
assert.match(indexSource, /FWHMは計算結果から求める値です/);
assert.match(indexSource, /装置の設定厚との対応は校正しておらず/);
...
assert.match(appSource, /configuredOverlayBounds\(result, PROFILE_TAIL_DISPLAY_BOUNDS\.minimum,/);
...
assert.match(appSource, /PROFILE_TAIL_DISPLAY_BOUNDS = Object\.freeze\(\{ yMin: -2\.08, yMax: 0\.08, minimum: 0\.01 \}\)/);
...
assert.match(appSource, /drawDetectorRowLegend\(ctx, diagram, left, top \+ 38, width\)/);
assert.match(appSource, /drawDiagramFamilyLegend\(ctx, diagram, left, top \+ 76, width\)/);
assert.match(appSource, /同じ取得データの重みを合算。軌道の重なりは混色。/);
...
assert.match(appSource, /合計重み w \(FW=0\)/);
assert.match(appSource, /全\$\{diagram\.totalRows\}列の候補軌道/);
...
assert.match(englishAppBundle, /Total weight w \(FW=0\)/);
assert.match(englishAppBundle, /Weights sum per acquired sample; trace overlaps blend/);
assert.match(englishAppBundle, /Candidate trajectories: all/);
