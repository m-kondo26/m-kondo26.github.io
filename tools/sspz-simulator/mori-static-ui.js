import {moriStaticConfig,moriStaticView,moriStaticAngleProfile,moriStaticCalculate,moriStaticProgress} from './mori-static-core.js?v=20260918-11';
...
let c=moriStaticConfig({focalSizeMm:query.has('focus')&&Number(query.get('focus'))===0?0:1.2});
...
function stateUrl(){const u=new URL(location.href);u.search='';if(en)u.searchParams.set('lang','en');u.searchParams.set('row',row+1);u.searchParams.set('r',radius);u.searchParams.set('angle',displayedAngle());u.searchParams.set('focus',c.focalSizeMm);return u;}
...
function displayView(){const shown=displayedAngle(),v=moriStaticView(c,{row,radius,angleDeg:shown});$('angle-value').value=`${shown}°`;$('angle-value').textContent=`${shown}°`;$('angle').value=shown;
  $('scope').textContent=tr(`静止寝台・16列 × 2 mm・${c.focalSizeMm>0?'有限焦点 1.2 mm':'点焦点'}の検証用モデル。本書の計算手順に沿うもので、掲載曲線の厳密な再現ではありません。`,`Static table, 16 × 2 mm rows, ${c.focalSizeMm>0?'1.2 mm finite focus':'point focus'}. A verification model following the book’s procedure, not an exact reproduction of its published curves.`);
...
function recalculate(){const ticket=++calculationId;result=null;progressCache.clear();document.body.dataset.ready='false';$('error').hidden=true;$('download').disabled=true;$('save-svg').disabled=true;$('play').disabled=true;$('families').setAttribute('aria-busy','true');$('families').replaceChildren();const status=document.createElement('p');status.setAttribute('role','status');status.textContent=tr('焦点条件を反映して、16か所の応答を計算しています…','Calculating all 16 planes with the selected focus…');$('families').append(status);$('instant').replaceChildren();$('accumulation').replaceChildren();$('progress-label').textContent=tr('計算中…','Calculating…');displayView();
  setTimeout(()=>{if(ticket!==calculationId)return;try{result=moriStaticCalculate(c);drawFamilies();displayView();$('download').disabled=false;$('save-svg').disabled=false;$('play').disabled=false;document.body.dataset.ready='true';}catch(e){$('error').hidden=false;$('error').textContent=tr('計算を完了できませんでした：','Calculation failed: ')+e.message;$('families').setAttribute('aria-busy','false');$('play').disabled=true;console.error(e);}},30);
}
