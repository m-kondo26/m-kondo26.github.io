...
The subsequent slice-centred angular interval, fixed point with moving evaluation plane, rectangular T average, normalization and native-sample width measurement are unchanged. The focal-state sequence is fixed relative to the acquired view index as the start angle changes through 360 conditions. Full 3D FBP, image reconstruction, scanner validation, and a proof of artifact suppression remain outside scope.
