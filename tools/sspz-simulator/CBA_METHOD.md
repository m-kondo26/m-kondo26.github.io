...
Candidate trajectories use the rebinned parallel angle theta, not the acquired fan-beam source angle beta. Samples are identified by unwrapped rebinned view and acquired row. The display folds angles into 0–360 degrees while retaining different-turn samples separately. For a nonzero axial image average, weights for the transverse sphere-centre location are accumulated across the averaging window using the exact coefficients of the same piecewise-linear integral used in reconstruction. Multiplying the recorded coefficients by their filtered values and angular factor reproduces that centre image sample. This is a trace of filtered row weights, not total raw-cell contributions to the entire SSPz or all ROI voxels. Marker area shows weight; at most about 72 acquired angular positions per turn are drawn. Exports retain every sample.
