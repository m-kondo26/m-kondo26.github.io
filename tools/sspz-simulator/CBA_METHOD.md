...
The acquired detector and point-projection definition are identical to `FDK_METHOD.md`: source radius R, row count N, row width d, pitch p, table feed H=pNd. Source angle beta has source position (R cos beta, R sin beta, H(beta-phi0)/(2 pi)). A detector ray has horizontal direction (-cos(beta-gamma), -sin(beta-gamma)); w is its axial coordinate on the source-centred cylinder of radius R. The ideal-point signal is integrated analytically over the finite detector aperture (POINT_RESPONSE_METHOD.md). Only discrete acquired views and channels are used in rebinning.
